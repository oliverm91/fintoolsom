...
class Coupon:
    def __init__(self, amortization: float, interest: float, residual: float, start_date: date, end_date: date, coupon_rate_convention: RateConvention):
        ...
        self.coupon_rate = Rate.get_rate_from_wf(self.wf, self.start_date, self.end_date, coupon_rate_convention)
        
    def get_accrued_interest(self, date: date, accrue_rate: Rate=None) -> float:
        accrue_rate = self.coupon_rate if accrue_rate is None else accrue_rate
...
class Coupons:
    ...
    def get_coupon_rate(self) -> float:
        return self.coupons[0].coupon_rate
...
class Bond:
    def __init__(self, **kwargs):
        ...
        self.coupon_rate = self.coupons.get_coupon_rate()
