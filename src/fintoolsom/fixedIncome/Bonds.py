...
class Coupons:
    ...
    def adjust_to_notional(self, notional: float):
        ...
        if self.check_residuals:
            assert round(float(n), 1) / round(float(notional), 1) - 1 < 1 / 100, (
...
class Bond:
    def __init__(self, **kwargs):
        ...
        self.coupons.adjust_to_notional(
            100
        )  # Everything is calculated to N = 100. Then adjusted to notional. This makes Bond is made with coupons with base 100.
    ...
    def copy(self) -> Self:
        return Bond(
            coupons=self.coupons.copy(), currency=self.currency, notional=self.notional
        )
