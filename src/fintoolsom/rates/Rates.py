...
class Rate:
    ...
    def get_wealth_factor(self, start_date: date, end_date: date) -> np.ndarray | float:
        return self._wf_router[self.rate_convention.interest_convention](self, self.rate_value, start_date, end_date)

    def get_discount_factor(self, start_date: date, end_date: date) ->  np.ndarray | float:
        return 1 / self.get_wealth_factor(start_date, end_date)
        
    def get_time_fraction(self, start_date: date, end_date: date) -> np.ndarray | float:
        return dates.get_time_fraction(start_date, end_date, self.rate_convention.day_count_convention, self.rate_convention.time_fraction_base)
    ...
    def _get_compounded_rate_values_from_wf(wf: float | np.ndarray, start_date: date | list[date] | np.ndarray, end_date: date | list[date] | np.ndarray, rate_convention: RateConvention):
        time_fraction = dates.get_time_fraction(start_date, end_date, rate_convention.day_count_convention, rate_convention.time_fraction_base)
        return np.power(wf, 1/time_fraction) - 1
    ...
    @staticmethod
    def get_rate_from_wf(wf: list[float] | np.ndarray | float, start_date: list[date] | np.ndarray | date, end_date: list[date] | np.ndarray | date, rate_convention: RateConvention) -> list[Self] | Self:
        ...
        return func_rate(rate_values, rate_convention)

    @staticmethod
    def get_rate_from_df(df: list[float] | np.ndarray | float, start_date: list[date] | np.ndarray | date, end_date: list[date] | np.ndarray | date, rate_convention: RateConvention) -> list[Self] | Self:
        return Rate.get_rate_from_wf(1/df, start_date, end_date, rate_convention)


def df_to_rate(discount_factor: float, start_date: date, end_date: date, rate_convention: RateConvention) -> Rate:
    r_aux = Rate(RateConvention(), 0) # is instantiation needed?
