...
class ZeroCouponCurve:
    ...
    df_interpolation_method: InterpolationMethod = field(
        default=InterpolationMethod.HermiteCubicSpline
    )
        ...
        if not self.curve_points and self.date_dfs:
            ...
            self.curve_points = [
                ZeroCouponCurvePoint(
                    t_i,
                    Rate(
                        self._rate_conv_for_dfs_init,
                        ic.get_rate_from_df(
                            df, dcc.get_time_fraction(self.curve_date, t_i, tfb)
                        ),
                    ),
                )
            ...
            raise ValueError(
                "If curve_points is not set, then date_dfs (list[tuples[date, float]]) must be set."
            )

        if not isinstance(self.df_interpolation_method, InterpolationMethod):
            raise TypeError(
                f"'df_interpolation_method' must be of type 'InterpolationMethod'. Received type '{type(self.df_interpolation_method)}' with value '{self.df_interpolation_method}'."
            )
    ...
    def set_df_curve(self):
        self.wfs = np.array(
            [
                cp.rate.get_wealth_factor(self.curve_date, cp.date)
    ...
    def add_point(self, curve_point: ZeroCouponCurvePoint):
        if curve_point.date < self.curve_date:
            raise ValueError(
                f"Cannot add point with date before curve date. Curve date: {self.curve_date}, point date: {curve_point.date}"
            )
    ...
    def get_dfs(self, dates_t: list[date]) -> np.ndarray:
        ...
        if small_tenors_amount:
            dfs[:small_tenors_amount] = self.curve_points[0].rate.get_discount_factor(
                self.curve_date, dates_t[:small_tenors_amount]
            )
        if greater_tenors_amount:
            dfs[-greater_tenors_amount:] = self.curve_points[
                -1
            ].rate.get_discount_factor(
                self.curve_date, dates_t[-greater_tenors_amount:]
            )
        ...
        if self.df_interpolation_method == InterpolationMethod.HermiteCubicSpline:
            ...
            raise NotImplementedError(
                f"Interpolation method: '{self.df_interpolation_method}' currently not implemented."
            )
    ...
    def get_dfs_fwds(
        self, start_dates: list[date], end_dates: list[date]
    ) -> np.ndarray:
        if len(start_dates) != len(end_dates):
            raise ValueError(
                f"Start and end dates must have the same length. Start dates: {start_dates}, end dates: {end_dates}"
            )
    ...
    def get_zero_rates(self, rate_convention: RateConvention = None) -> list[Rate]:
        if rate_convention is None:
            return [cp.rate.copy() for cp in self.curve_points]
        else:
            rates_obj = []
            for cp in self.curve_points:
                r = cp.rate.copy()
                if r.rate_convention == rate_convention:
                    ...
                    r.convert_rate_convention(rate_convention, self.curve_date, cp.date)
                    rates_obj.append(r)
            return rates_obj
    ...
    def get_zero_rate(self, date: date, rate_convention: RateConvention = None) -> Rate:
        ...
        df = self.get_df(date)
        yf = rate_convention.day_count_convention.get_time_fraction(
            self.curve_date, date, rate_convention.time_fraction_base
        )
        r = rate_convention.interest_convention.get_rate_from_df(df, yf)
        return Rate(rate_convention, r)

    def get_zero_rates_values(
        self, rate_convention: RateConvention = None
    ) -> np.ndarray:
        rates_obj = self.get_zero_rates(rate_convention)
        return np.array([r.rate_value for r in rates_obj])

    def get_accrued_interest(
        self, notional: float, start_date: date, end_date: date
    ) -> float:
        return notional * (self.get_wf_fwd(start_date, end_date) - 1)

    def get_accrued_interests(
        self, notional: np.ndarray, start_dates: list[date], end_dates: list[date]
    ) -> np.ndarray:
    ...
    def get_aged_curve(self, date: date) -> Self:
        if date <= self.curve_date:
            raise ValueError(
                f"Date {date} must be posterior to curve date {self.curve_date}."
            )
        if date > self.curve_points[-1].date:
            raise ValueError(
                f"Date {date} must be previous to curve last ZeroCouponCurvePoint date {self.curve_points[-1].date}."
            )
    ...
    def copy(self) -> Self:
        return ZeroCouponCurve(
            self.curve_date, [zccp.copy() for zccp in self.curve_points]
        )
