...
class Option(ABC):
    ...
    def get_mtm(
        ...
        mtm = (
            ...
            * self.notional
            * (
                spot * math.exp(-q * yf) * norm.cdf(self._sign * d1)
                - self.strike * math.exp(-r * yf) * norm.cdf(self._sign * d2)
            )
        )
        return mtm
    ...
    def get_delta(
        ...
        delta = (
            self._sign * self.notional * math.exp(-q * yf) * norm.cdf(self._sign * d1)
        )
        return delta
    ...
    def get_gamma(
        ...
        _, q = self._get_rates(t, domestic_curve, foreign_curve)
        yf = self._get_yf(t)
        d1, vol_sqrt_yf = self._get_d1(
            t, spot, volatility, domestic_curve, foreign_curve, return_vol_sqrt_yf=True
        )
    ...
    def _get_d1(
        ...
        d1 = (
            math.log(spot / self.strike) + (r - q + volatility * volatility / 2) * yf
        ) / vol_sqrt_yf
    ...
    def _get_rates(
        self, t: date, domestic_curve: ZeroCouponCurve, foreign_curve: ZeroCouponCurve
    ) -> tuple[float, float]:
        df_r = domestic_curve.get_df(self.maturity)
        rc = (
            _default_rc
            if self._valuation_rate_convention is None
            else self._valuation_rate_convention
        )
        interest_conv = rc.interest_convention
        yf = rc.day_count_convention.get_time_fraction(
            t, self.maturity, rc.time_fraction_base
        )
    ...
    def _get_both_ds(
        ...
        return self._get_d2(
            t, spot, volatility, domestic_curve, foreign_curve, return_both_ds=True
        )
    ...
    def get_strike_from_delta(
        ...
        yf = rate_convention.day_count_convention.get_time_fraction(
            t, maturity, rate_convention.time_fraction_base
        )
...
class Call(Option):
    ...
    def get_strike_from_delta(
        ...
        return Option.get_strike_from_delta(
            delta, spot, volatility, domestic_curve, foreign_curve, maturity, 1
        )
